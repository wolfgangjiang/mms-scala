...
object Log {
  ...
  def error(module_name : String, e : Exception) : Unit = {
    ...
    error(module_name, sw.toString.split("\n")(0))
  }

  def bytes(module_name : String, message: String) : Unit = 
    write("bytes", module_name, message)

  def warn_unhandled_packet(module_name : String, 
                            packet : List[Byte]) : Unit = {
    error(module_name, 
          "warning of unhandled packet: " + 
          SessionHelpers.to_hex_string(
            SessionHelpers.decode_0x7d_escape(packet)))
  }
}
