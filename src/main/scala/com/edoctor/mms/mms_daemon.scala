    ...
    try {
      ...
      while(going) {
        ...
        if(request == null) {
          ...
          val req_obj = MmsPdu.parse_request(request)
          if(req_obj.isRight && req_obj.right.get.date == today_string)
...
object MmsDaemon {
  ...
  private def make_mms_message(request : String) : List[Byte] = {
    val req_obj = MmsPdu.parse_request(request)
    if(req_obj.isLeft) {
      Log.error("make_mms_message", req_obj.left.get)
      throw new MalformedMessageException
    }
    MmsPdu.compose(req_obj.right.get)
  }
  ...
  private def is_of_future(request : String) : Boolean = {
    ...
    try {
      ...
      if(req_obj.isLeft) {
        ...
        val date = req_obj.right.get.date
        (date(0).isDigit && date > today_string)
      }
    } catch {
      case e : Exception => 
        return false
